...
const MyWork = () => {
  ...
  // Helper for conditional linking, now using navigate
  const ProjectLinkWrapper = ({ project, children }) => {
    ...
    // Use navigate to go to the page and pass the githubLink in the state
    return (
        <div 
          className="cursor-pointer" 
          onClick={() => navigate('/not-live', { state: { githubLink: project.githubLink } })}
        >
          {children}
        </div>
      );
          ...
          <div className="lg:col-span-2">
            {projects.length > 0 ? (
                    ...
                    >
                      <div className="p-1">
                        <Card className="bg-neutral-900 border-neutral-800 transition-colors duration-300 group-hover:border-purple-500">
                          <CardContent className="flex flex-col gap-4 aspect-[4/3] p-4 items-start justify-between">
                            ...
                            <p className="text-sm text-neutral-400 flex-grow">{project.description}</p>
                            <div className="flex gap-4">
                              {project.githubLink && <a href={project.githubLink} target="_blank" rel="noopener noreferrer" className="text-neutral-300 hover:text-purple-400">GitHub</a>}
                              {project.liveLink && <a href={project.liveLink} target="_blank" rel="noopener noreferrer" className="text-neutral-300 hover:text-orange-400">Live Demo</a>}
                            </div>
                          </CardContent>
                        </Card>
                      </div>
              ...
              <div className="flex items-center justify-center h-full aspect-video bg-neutral-900 rounded-lg">
                <p className="text-center text-neutral-400 text-xl">Loading projects...</p>
              </div>
            )}
          </div>
        </div>
